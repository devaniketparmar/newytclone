...
- [ ] **Advanced Analytics** - Detailed user behavior analytics
    - [x] **Watch Time Tracking** - Detailed watch time metrics (partial)
      - [x] Server: Added POST `/api/videos/[id]/watchtime` to accept watch-time heartbeats and upsert/update `views` records (watchDuration). (Implemented)
      - [x] Client: Instrumented `VideoPlayer` to send periodic heartbeats (every ~15s) and a final heartbeat on pause/ended; sends seconds watched and player position. (Implemented)
      - [ ] Aggregation: Upsert daily `video_analytics` / `channel_analytics` from `views` (TODO)
      - [ ] UI: Wire analytics dashboard pages to the aggregated watch-time metrics (TODO)
